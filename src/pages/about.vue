<template>
	<div>
		<Nav />
		<main class="container about">
			<div class="main-info">
				<h2 class="title">O Leadux</h2>
				<p class="desc">
					Leadux — это площадка для развития профессиональных навыков в сфере
					IT, которая предоставляет доступ к структурированному материалу в
					формате видеоуроков, заданий и полезных ресурсов. Мы уделяем особое
					внимание качеству и доступности контента, предлагаем интересные
					домашние задания в игровой форме и огромное количество полезных
					ссылок, отобранных профессионалами за многие годы работы. Специалисты
					Leadux оказывают помощь студентам на всех этапах обучения и после
					него, дают профессиональные советы и помогают с трудоустройством.
				</p>
			</div>

			<div class="author">
				<div class="col-l">
					<picture>
						<source
							class="person"
							srcset="@/assets/img/author1.5x.webp"
							type="image/svg"
						/>
						<img
							src="@/assets/img/author1.5x.png"
							alt="Фото Игорь Т."
							class="person"
						/>
					</picture>
				</div>
				<div class="col-r">
					<span class="status">Автор курса Figma</span>
					<span class="name">Igor T.</span>
					<p class="desc">
						Игорь является экспертом в области UI/UX с 12-ти летним опытом
						работы. Featured Deigner на Behance и Upwork. Есть работы в
						курируемых галереях Adobe. С недавнего времени ведет канал на
						Youtube, посвященный фотошопу.
					</p>
					<ul class="list">
						<li class="item">
							<a
								href="https://www.behance.net/kenezibo"
								class="link behance"
								target="_blank"
								rel="noopener noreferrer"
								>Behance</a
							>
						</li>
						<li class="item">
							<a
								href="https://www.youtube.com/channel/UC2UB7pz_KZbnR2GrwBPAARg"
								class="link youtube"
								target="_blank"
								rel="noopener noreferrer"
								>Youtube</a
							>
						</li>
						<li class="item">
							<a
								href="https://www.upwork.com/o/profiles/users/~0153d33098d92e08e6/"
								class="link upwork"
								target="_blank"
								rel="noopener noreferrer"
								>Upwork</a
							>
						</li>
					</ul>
				</div>
			</div>
		</main>
		<Footer />
	</div>
</template>

<script>
import Nav from '@/components/sections/Nav'
import Footer from '@/components/sections/Footer.vue'
export default {
	components: {
		Nav,
		Footer,
	},
}
</script>

<style lang="postcss" scoped>
.about {
	.main-info {
		.title {
			font-weight: bold;
			font-family: 'Freigeist-XWideBold', system-ui;
			font-size: responsive 1.75rem 2.75rem;
			text-align: center;
			color: #fff;
			margin-top: 3.5rem;
			margin-bottom: 1.5rem;
		}

		.desc {
			font-weight: 100;
			line-height: 160%;
			color: #e6e6e6;
			max-width: 830px;
			width: 100%;
			margin: 0 auto 8rem;
		}
	}

	.author {
		width: 100%;
		max-width: 1008px;
		background-color: #1a1a1a;
		border-radius: 13px;
		margin: 0 auto;
		padding: 1.8rem 0 2rem;
		display: grid;
		grid-template-columns: 355px 1fr;

		@media (max-width: 825px) {
			grid-template-columns: 280px 1fr;
		}

		@media (max-width: 695px) {
			display: block;
			margin-top: 350px;
		}

		.col-l {
			position: relative;

			@media (max-width: 695px) {
				&::after {
					content: '';
					background: linear-gradient(180deg, rgba(0, 0, 0, 0) 50%, #000 100%);
					width: 100%;
					height: 250px;
					position: absolute;
					bottom: 27px;
					left: 0;
					z-index: -1;
				}
			}

			.person {
				width: 100%;
				max-width: 255px;
				position: absolute;
				bottom: -2rem;
				left: 50%;
				transform: translateX(-50%);
				filter: grayscale(100%);
				transition: filter 0.3s linear;
				z-index: 1;

				@media (min-width: 768px) {
					&:hover {
						filter: grayscale(0%);
					}
				}
				@media (max-width: 768px) {
					filter: grayscale(0%);
				}
				@media (max-width: 695px) {
					z-index: -1;
					bottom: -5rem;
				}
			}
		}

		.col-r {
			padding-right: 1rem;
			position: relative;

			&::after {
				content: '';
				width: 30px;
				height: 40px;
				background-image: url('data:image/svg+xml;base64,');
				background-size: cover;
				position: absolute;
				top: -29px;
				right: 10%;
			}

			@media (max-width: 695px) {
				padding-left: 1rem;
			}

			.status {
				padding: 10px 22px;
				background-color: #222;
				border-radius: 7px;
				font-weight: 500;
				font-size: 14px;
				line-height: 140%;
				color: #888;
				margin-bottom: 1.4rem;
				display: inline-block;
			}

			.name {
				display: block;
				font-weight: bold;
				font-size: 1.5rem;
				line-height: 140%;
				color: #ffffff;
				margin-bottom: 1rem;
			}

			.desc {
				font-size: 1rem;
				line-height: 149%;
				color: #aeaeae;
				max-width: 576px;
				margin-bottom: 1.5rem;
			}

			.list {
				list-style: none;
				padding: 0;
				margin: 0;
			}

			.item {
				display: inline-block;

				&:not(:last-child) {
					margin-right: 2.5rem;
				}

				@media (max-width: 695px) {
					display: block;
					&:not(:last-child) {
						margin-bottom: 1rem;
					}

					@media (max-width: 825px) {
						margin-right: 1rem;
					}
				}
			}

			.link {
				display: inline-block;
				color: #5b5b5b;
				cursor: pointer;
				position: relative;
				padding-left: 2rem;
			}

			.behance {
				&:before {
					content: '';
					width: 19px;
					height: 14px;
					background-image: url('data:image/svg+xml;base64,');
					background-size: cover;
					position: absolute;
					top: 45%;
					left: 0;
					transform: translateY(-50%);
				}
			}
			.youtube {
				&::before {
					content: '';
					width: 20px;
					height: 17px;
					background-image: url('data:image/svg+xml;base64,');
					background-size: cover;
					position: absolute;
					top: 45%;
					left: 0;
					transform: translateY(-50%);
				}
			}

			.upwork {
				&::before {
					content: '';
					width: 24px;
					height: 20px;
					background-image: url('../assets/img/upwork.png');
					background-size: cover;
					position: absolute;
					top: 45%;
					left: 0;
					transform: translateY(-50%);
				}
			}
		}
	}
}
</style>
