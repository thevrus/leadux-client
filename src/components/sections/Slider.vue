<template>
	<section class="container">
		<div class="row">
			<span>
				<h3>Зарплата UX/UI дизайнера</h3>
				<p>Источник данных: <a href="#!" target="_blank">Хабр Карьера</a></p>
			</span>

			<output :class="{ small: value == max }"
				>{{ val }}<span v-show="value != max"> $</span></output
			>
		</div>

		<div>
			<span class="position">Junior</span>
			<span class="position">Middle</span>
			<span class="position">Senior</span>
		</div>

		<input
			type="range"
			:min="min"
			:max="max"
			:step="step"
			v-model="value"
			@change="slide"
			class="range pink"
		/>
	</section>
</template>

<script>
export default {
	data() {
		return {
			min: 36000,
			max: 170000,
			step: 1,
			value: 100000,
		}
	},
	computed: {
		val() {
			return this.value == this.max ? 'Быть дизайнером – бесценно' : this.value
		},
	},
	methods: {
		slide(event) {
			this.value = event.target.value
		},
	},
}
</script>

<style lang="postcss" scoped>
section {
	margin-bottom: 200px;
}

.row {
	display: flex;
	justify-content: space-between;
	align-items: baseline;
}

output {
	font-weight: bold;
	font-size: 52px;

	span {
		font-style: normal;
		font-weight: bold;
		font-size: 28px;
		line-height: 120%;
	}
}

.small {
	font-size: 28px;
	padding: 1.45rem 0;
}

h3 {
	font-style: normal;
	font-weight: 500;
	font-size: 44px;
	line-height: 120%;
	color: #fff;
	margin: 0 0 1rem 0;
}

p {
	font-style: normal;
	font-weight: 500;
	font-size: 18px;
	line-height: 120%;
	color: #cccccc;

	a {
		font-style: normal;
		font-weight: 500;
		font-size: 18px;
		color: #fff;
		text-decoration: underline;
	}
}

div {
	margin-top: 5rem;
	display: grid;
	grid-template-columns: repeat(3, 1fr);
	color: #fff;

	.position {
		position: relative;
		font-size: 1.2rem;

		&:not(:first-child)::after {
			content: '';
			display: inline-block;
			position: absolute;
			bottom: -42px;
			left: 0;
			width: 14px;
			height: 2px;
			background-color: #000;
		}
	}
}

.range {
	margin: 2.5rem 0;
	appearance: none;
	width: 100%;
}

/* Slider */
input[type='range']::-webkit-slider-runnable-track {
	appearance: none;
	background-color: #343434;
	background-color: #e937b3;
	height: 2px;
	cursor: pointer;
}

input[type='range']:focus {
	outline: none;
}

input[type='range']::-moz-range-track {
	appearance: none;
	background-color: #343434;
	background-color: #e937b3;
	height: 2px;
}

input[type='range']::-webkit-slider-thumb {
	appearance: none;
	border-radius: 50%;
	height: 64px;
	width: 64px;
	bottom: 32px;
	background-color: #e937b3;
	filter: drop-shadow(0px 0px 16px rgba(233, 55, 179, 0.5));
	position: relative;
	cursor: pointer;
	transition: filter 0.3s ease;
	background-image: url('data:image/svg+xml;base64,');
	background-repeat: no-repeat;
	background-position: center center;

	&:active {
		filter: drop-shadow(0px 0px 24px rgba(233, 55, 179, 0.5));
	}

	&:after {
		content: '22';
	}
}

input[type='range']::-moz-range-thumb {
	appearance: none;
	border-radius: 50%;
	height: 64px;
	width: 64px;
	bottom: 32px;
	background-color: #e937b3;
	filter: drop-shadow(0px 0px 20px rgba(233, 55, 179, 0.5));
	position: relative;
	cursor: pointer;
	transition: transform 0.3s ease;
	background-image: url('data:image/svg+xml;base64,');
	background-repeat: no-repeat;
	background-position: center center;

	&:active {
		transform: scale(0.95);
	}
}

input[type='range']::-webkit-slider-thumb:active {
	cursor: pointer;
}

input[type='range']::-moz-range-thumb:active {
	cursor: pointer;
}
</style>
